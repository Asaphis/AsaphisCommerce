                ...
                <div className="flex flex-col">
                    ...
                    <div className="flex items-center mt-4 gap-4">
                        <button 
                            ...
                            className={`flex items-center justify-center gap-2 w-full py-3 border rounded-lg transition ${
                                isInWishlist(productData._id) 
                                    ? 'bg-red-50 border-red-200 text-red-700' 
                                    : 'border-gray-300 text-gray-700 hover:bg-gray-50'
                            }`}
                        >
                            <FiHeart className={`w-4 h-4 ${isInWishlist(productData._id) ? 'fill-current' : ''}`} />
                            {isInWishlist(productData._id) ? 'In Wishlist' : 'Add to Wishlist'}
                        </button>
                        <button 
                            onClick={() => {
                                const currentUrl = new URL(window.location.origin + '/compare');
                                currentUrl.searchParams.set('products', productData._id);
                                router.push(currentUrl.toString());
                            }}
                            className="flex items-center justify-center gap-2 w-full py-3 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg transition"
                        >
